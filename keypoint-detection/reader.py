...
def augment(images, labels):
	...
	for label in labels:
		...
		for i in range(NUM_POINTS):
			keypoints.append(ia.Keypoint(
				x=label[i*NUM_DIMS]*MODEL_WIDTH,
				y=label[i*NUM_DIMS+1]*MODEL_HEIGHT))
		keypoints_on_images.append(ia.KeypointsOnImage(
			keypoints,
			shape=(MODEL_HEIGHT,MODEL_WIDTH,3)))
	...
	for idx1, keypoints_after in enumerate(keypoints_aug):
		for idx2, keypoint in enumerate(keypoints_after.keypoints):
			if not (labels[idx1, idx2*NUM_DIMS] == 0 and labels[idx1, idx2*NUM_DIMS+1] == 0):
				labels[idx1, idx2*NUM_DIMS] = float(keypoint.x / MODEL_WIDTH)
				labels[idx1, idx2*NUM_DIMS+1] = float(keypoint.y / MODEL_HEIGHT)
